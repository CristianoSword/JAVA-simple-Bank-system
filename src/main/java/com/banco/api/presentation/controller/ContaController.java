...
public class ContaController {
    ...
    @Operation(summary = "Obter extrato de uma conta, listando todas as transações.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Extrato gerado com sucesso"),
            @ApiResponse(responseCode = "404", description = "Conta não encontrada")
    })
    @GetMapping("/{contaId}/extrato")
    public ResponseEntity<List<TransacaoResponse>> extrato(@PathVariable Long contaId) {
        List<TransacaoResponse> extrato = transacaoService.extratoPorConta(contaId);
